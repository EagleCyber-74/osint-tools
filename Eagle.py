...
@is_option
def IP_Track():
    ip = input(f"{Wh}\n MASUK KAN IP TARGET : {Gr}")  # INPUT IP ADDRESS
    print()
...
def phoneGW():
    User_phone = input(
        f"\n {Wh}Masuk kan nomor target {Gr}Ex [+6281xxxxxxxxx] {Wh}: {Gr}")  # INPUT NUMBER PHONE
    ...
    try:
        ...
        for site in social_media:
            ...
            if response.status_code == 200:
                ...
                results[site['name']] = (f"{Ye}Username tidak di temuka nyet. {Ye}!")
    except Exception as e:
        print(f"{Re}Error : {e}")
        return

    print(f"\n {Wh}========== {Gr}TAMPILKAN INFORMASI USERNAME{Wh}==========")
    print()
...
def showIP():
    ...
    print(f"\n {Wh}========== {Gr}TAMPILKAN INFORMASI IP KAMU {Wh}==========")
    print(f"\n {Wh}[{Gr} + {Wh}] Ip kamu : {Gr}{Show_IP}")
    print(f"\n {Wh}==============================================")
...
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
              {Wh} •_•  C O D E   B Y  EAGLECYBERLCR  •_•
    """)
